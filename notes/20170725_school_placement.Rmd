...
placed <- join_zone_bio %>% ungroup() %>% filter(schools == "mid") %>% select(student_id_scram, year, dbn)
colnames(placed)[3] <- "mid_dbn"
...
where_students_go_feeder_data <- join_mid_hs %>% group_by(mid_dbn, hs_dbn) %>% summarise(count = n())
