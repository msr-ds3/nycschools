...
hs_school <- max_year_min_year %>% filter(!is.na(hs)) %>% select(student_id_scram, hs)

mid_hs_data <- inner_join(mid_school, hs_school, by = "student_id_scram")

max_year_min_year <- max_year_min_year %>%group_by(student_id_scram) %>% summarise(mid = max(mid), hs = max(hs))

#only keep entry to each school level for each student
bio_distinct_schools <- test %>% 
  group_by(student_id_scram, schools) %>% 
  filter(year == min(year))

####################################################################################################
#id: 999962962 all years
#view
...
#############################
# 2,208,271 rows of students in all years but 2005 and 2006

#numbers of students that appear n number of times
all_bios %>% filter(year >2006) %>% group_by(student_id_scram) %>% summarise(count = n()) %>% group_by(count) %>% summarise(countx = n())
...
#9,502,347 rows in distict zones
distinct_zones

nrow(distinct_zones %>% filter(year != 2007))
...
save(zoneData, file = "/data/nycdoe/clean_data/common_data_frames/zoneData.Rdata")

zoneData <- zoneData %>% select(1:8)
...
join_zone_bio <- inner_join(bio_distinct_schools, zoneData, by = c( "student_id_scram", "year"))

join_zone_bio

join_zone_bio %>% select(student_id_scram, year, em, hs, mid) %>% gather("schooltype","dbn", em, hs, mid) %>% filter(!is.na(dbn))


save(join_zone_bio, file = "/data/nycdoe/clean_data/join_zone_bio.Rdata")
```
...
midDbn_hs_zone_data <- join_zone_bio_matches %>%filter(!is.na(mid)) %>% mutate(zoned_boro = substr(zoned_hs_dbn, 3,3))

#this is for feeder schools
save(midDbn_hs_zone_data, file = "/data/nycdoe/clean_data/midDBN_hs_zone_eighth_grade_placement.Rdata")
...
local_zones_data %>% mutate(boro = substr(zoned_elm_dbn, 3, 3)) %>% 
  filter(!is.na(attending_zoned_dbn)) %>% 
  group_by(boro) %>% 
...
local_zones_data %>% ggplot(aes(x  = perc_local)) + geom_histogram() + xlab("Number of schools")

local_zones_data %>% ggplot(aes(y = perc_local, x = zoned_elm_dbn)) + geom_density() 
```
