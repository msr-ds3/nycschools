---
title: "20170731 - Do You Get What You Want (cont. 3)"
author: "Thoa"
date: "7/31/2017"
...
# NEW DATA: load in data on students' performance (what `performance` means for students: first, percentile student's scores against their peers in the same year and the same grade; then, take average of all available percentiles - "available" because some students will NOT take all kinds of tests out there)
...
rm(perstudent_gpa_and_perf_long)
perstudent_gpa_and_perf_long <-
  perstudent_gpa_long %>%
  left_join(avg_all_percentile, by = c("student_id_scram", "year"))
# we're afraid the grade_level in 2 tables do not match, but in fact, grade_level.x simply covers more cases than grade_level.y
head(perstudent_gpa_and_perf_long)
tail(perstudent_gpa_and_perf_long)
summary(perstudent_gpa_and_perf_long)

# this code proves that NA in grade_level.y already encompasses NA in grade_level.x. In other words, grade_level.x provides more coverage. 
perstudent_gpa_and_perf_long %>%
  filter(is.na(grade_level.x) & !is.na(grade_level.y))
# this code shows that when they are both non-NA, grade_level.x and grade_level.y match! Good news!
perstudent_gpa_and_perf_long %>%
  filter(!is.na(grade_level.x) & !is.na(grade_level.y) & grade_level.x != grade_level.y)
# hence we will keep grade_level.x and drop grade_level.y
perstudent_gpa_and_perf_long <-
  perstudent_gpa_and_perf_long %>% 
  select(-grade_level.y, "grade_level" = grade_level.x)
```

```{r load-student-demo, results='hide', message=TRUE}

# load in students' demographics from June Biog
read_demog <- function(filename) {
  year <- as.numeric(gsub('-.*$', '', basename(filename)))
  df <- read_csv(filename, col_types = cols(student_id_scram=col_character(),
...
demog_09_15 <- map_df(june_biog_files[5:11], read_demog)
```
...
# load in RANKED schools (i.e. schools and their percentile)
load('/data/nycdoe/clean_data/school_percentile_for_thoa.Rdata')
hs_features_new <-
  hs_features_new %>%
  full_join(school_percentile, by = "dbn")
summary(hs_features_new)
```

#### Programcode look-up

```{r load-programcode-dict}
load('/data/nycdoe/clean_data/schoolcodemapping_fuller.Rdata')
```
...
withfeatures_satisfied_09_15 <-
  left_join(satisfied_09_15, demog_09_15, by=c("student_id_scram", "year")) %>%
...
# this goes from 4,431,374 down to 4,425,928 (>5k rows dropped)
perstudent_gpa_and_perf_long <-
  perstudent_gpa_and_perf_long %>%
  filter(student_id_scram %in% withfeatures_satisfied_09_15$student_id_scram)
withfeatures_satisfied_09_15 <-
  left_join(withfeatures_satisfied_09_15, perstudent_gpa_and_perf_long, by=c("student_id_scram", "year")) %>%
...
# do these 13,896 missing-regent schools cover the 13,173 missing-percentile schools? YES!
withfeatures_fullr1r1_09_15 %>%
  filter(!is.na(r1r1_English_regents) & is.na(r1r1_percentile))

# drop 13,896 rows with NA's in high school's regents. This already encompasses the ~11k rows with NA's in demographics and attendance, and the 13,173 rows with NA's in school-percentile.
withfeatures_fullr1r1_09_15 <- 
  withfeatures_fullr1r1_09_15 %>%
  filter(!is.na(r1r1_English_regents))
```
...
# we will drop 70 rows with missing-percentile for feeder schools, and 2325 rows with missing-gradrate for high schools, and 9778 rows with missing-performance for the applicants. In total, 12,173 rows (2.59%) are dropped from the fullr1r1 table.
model_data <- 
  withfeatures_fullr1r1_09_15 %>%
  filter(!is.na(r1r1_perc_grad) & !is.na(feeder_percentile) & !is.na(performance))
summary(model_data)

frmula <- as.formula(was_satisfied ~ borough + sex + ethnicity + ell + swd + poverty + quantiled_GPA + performance + 
                                     ...
                                     feeder_borough + feeder_percentile + r1r1_percentile +
                                     r1programtype1 + r1r1_interest + r1r1_English_regents + 
