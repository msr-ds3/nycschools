...
set.seed(11)
tiny_sample_name <- ten_schools %>% select(student_id_scram) %>% unique() %>% sample_n(100)
...
borough_sample_name <- bios_by_borough %>% 
  ...
  group_by(borough)%>%
  sample_n(500)
borough_sample <- left_join(borough_sample_name,bios_by_borough, by=c("student_id_scram", "borough"))
...
ethnicity_sample_name <- bios_by_ethnicity %>% 
  ...
  unique() %>% sample_n(500)
ethnicity_sample <- left_join(ethnicity_sample_name,bios_by_ethnicity, by=c("student_id_scram", "ethnicity"))
...
gender_sample_name <- bios_by_gender %>% 
  ...
  unique() %>% sample_n(500)
gender_sample <- left_join(gender_sample_name,bios_by_gender, by=c("student_id_scram", "gender"))
